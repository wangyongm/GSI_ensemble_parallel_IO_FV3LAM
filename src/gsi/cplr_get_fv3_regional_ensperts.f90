...
use abstract_get_fv3_regional_ensperts_mod,only: abstract_get_fv3_regional_ensperts_class
  ...
  contains
    ...
    procedure, pass(this) :: general_read_fv3_regional_parallelIO
...
contains
  ...
  !   2022-04-01  Yongming Wang and X. Wang - add ensemble parallel IO capability for hybrid EnVar
  !                                     for both radar and conventional DA
...
22  format(a,'fv3SAR',i2.2,'_ens_mem',i3.3)
 ...
 ! DEFINE INPUT FILE NAME
          ...
          if(mype==0) then
             write(6,'(I0,A)') mype,': reading ensemble data in parallel is done (ens_fast_read=.true.)'
          endif
        end if
...
20 write(6,*) 'get_fv3_regional_ensperts_netcdf: read WRF-ARW ens failed ',n
  ...
  subroutine general_read_fv3_regional_parallelIO(this,iope,fv3_filenameginput,g_ps,g_u,g_v,g_tv,g_rh,g_oz, &
  ...
  !   2022-04-01 Yongming Wang and X. Wang - read all fields for each member for
  ...
  !$$$ end documentation block
    ...
    use gsi_rfv3io_mod, only: gsi_fv3ncdf_read_ens_parallelIO,gsi_fv3ncdf_readuv_ens_parallelIO
...
!   Declare local variables
    ...
    if( mype == iope )then
       ...
       if(fv3sar_ensemble_opt == 0 ) then  
         call gsi_fv3ncdf_readuv_ens_parallelIO(g_u,g_v,fv3_filenameginput,iope)
       else
       ...
       if(fv3sar_ensemble_opt == 0) then
         if (if_model_dbz) then
             call gsi_fv3ncdf_read_ens_parallelIO(fv3_filenameginput%dynvars,fv3_filenameginput,delp=g_delp,tsen=g_tsen,w=g_w,iope=iope)
             call gsi_fv3ncdf_read_ens_parallelIO(fv3_filenameginput%tracers,fv3_filenameginput,q=g_q,oz=g_oz,ql=g_ql,qr=g_qr,&
                                                  qs=g_qs,qi=g_qi,qg=g_qg,iope=iope)
             call gsi_fv3ncdf_read_ens_parallelIO(fv3_filenameginput%phyvars,fv3_filenameginput,dbz=g_dbz,iope=iope)
         else
             call gsi_fv3ncdf_read_ens_parallelIO(fv3_filenameginput%dynvars,fv3_filenameginput,delp=g_delp,tsen=g_tsen,iope=iope)         
             call gsi_fv3ncdf_read_ens_parallelIO(fv3_filenameginput%tracers,fv3_filenameginput,q=g_q,oz=g_oz,iope=iope)
         end if
       else
         write(6,*) "Warning: we can only grab fields from restart files not cold start files for ensemble!"
  ...
  return       
  end subroutine general_read_fv3_regional_parallelIO
  ...
  !   prgmmr: Yongming Wang and X. Wang             org: OU/MAP           date: 2022-04-01
  !
  ! abstract: All fields have been read in by general_read_fv3_regional_parallelIO.
  ...
  ! program history log:
  !                                      
  !   2022-04-01 Yongming Wang and X. Wang - Changed from the code for WRF-ARW
  !
  !   input argument list:
  !
  !   output argument list:
  ...
  ! program history log:
  !   2018-  Ting      - intial versions  
  !   2022-04-01 Yongming Wang and X. Wang - read in reflectivity for direct radar
  !                                    reflectivity assimilation for hybrid EnVar, poc: xuguang.wang@ou.edu
  !
  !   input argument list:
  !
  !   output argument list:
